# echo_v_engine.py
# AI-powered Echo V engine using smoothed RSI and Heikin Ashi candles
...
def detect_echo_v(df):
    try:
        ...
        if fast_prev < slow_prev and fast_last > slow_last:
            return {"status": "Echo V-Split Bullish", "strength": 1.5}
        elif fast_prev > slow_prev and fast_last < slow_last:
            return {"status": "Echo V-Split Bearish", "strength": 1.5}
        elif abs(fast_last - slow_last) < 1.0 and abs(fast_prev - slow_prev) < 1.0:
            return {"status": "Echo Sync Flat", "strength": 0.5}
        elif fast_last > slow_last and fast_prev > slow_prev:
            return {"status": "Echo Sync Up", "strength": 0.7}
        elif fast_last < slow_last and fast_prev < slow_prev:
            return {"status": "Echo Sync Down", "strength": 0.7}
        elif fast_last < 20 and fast_prev < 30 and slow_last < 30:
            return {"status": "Echo Collapse Bearish", "strength": 1.0}
        elif fast_last > 80 and fast_prev > 70 and slow_last > 70:
            return {"status": "Echo Collapse Bullish", "strength": 1.0}
...
def get_echo_v_signal(df):
    return detect_echo_v(df)

# --- Example Test ---
if __name__ == "__main__":
    import yfinance as yf
    btc = yf.download("BTC-USD", interval="1h", period="5d")
    result = get_echo_v_signal(btc)
    print("Echo Signal:", result)
